...
支持实时麦克风录制和音频文件流式传输（流式 VAD，实时出结果）。

用法:
    # 麦克风模式
    python src/client_python.py --server ws://localhost:8000/ws --mic

    # 文件模式
    python src/client_python.py --server ws://localhost:8000/ws --file data/2speakers_example.wav

    # 带热词
    python src/client_python.py --server ws://localhost:8000/ws --file audio.wav --hotwords "张三,李四,北京"

    # 关闭说话人分离显示
    python src/client_python.py --server ws://localhost:8000/ws --mic --no-spk
"""
...
async def run_mic(args):
    ...
    async with websockets.connect(args.server, ping_interval=None) as ws:
        ...
        # 请求说话人分离（默认开，--no-spk 关闭）
        await ws.send(f"SPK:{'true' if args.spk else 'false'}")
        await ws.recv()  # spk_set
